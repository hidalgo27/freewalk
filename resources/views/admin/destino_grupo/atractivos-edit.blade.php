            ...
            <div class="form-row">
                ...
                <div class="form-group col-12 text-left">
                    ...
                    @if (Storage::disk('destino_grupo')->has($destino_grupo_imagen->imagen))
                        <figure class="figure m-3" id="destino_grupo_atractivo_imagen_{{ $destino_grupo_imagen->id }}">
                            <img src="{{ route('admin.destino_grupo.get_imagen.path',$destino_grupo_imagen->imagen) }}" class="figure-img rounded w-100" alt="{{ $destino_grupo_imagen->imagen}}">
                            <figcaption class="figure-caption text-right mt-0">
                                <a href="#!" class="btn btn-danger btn btn-block" onclick="borrar_imagen_destino_inicio('destino_grupo_atractivo_imagen_{{ $destino_grupo_imagen->id}}')">
                                    <i class="fas fa-trash-alt"></i>
                                </a>
                            </figcaption>
                            <input type="hidden" name="atractivo_imagen_" value="{{ $destino_grupo_imagen->id }}">
                        </figure>
                    @endif
                </div>
                <div class="form-group col-12">
