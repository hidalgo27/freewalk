...
<li class="breadcrumb-item active" aria-current="page">Lista de preguntas</li>
@endsection
@section('content')
    <div class="card">
    ...
    <div class="card">
        <div class="card-header">
            <div class="row">
                <div class="col-8">
                    <h4 class="text-uppercase">lista de preguntas por destino grupo</h4>
                </div>
                <div class="col-4 text-right">
                    <a href="{{route('admin.destino-grupo.preguntas.create.path',$destino_grupo->id)}}" class="btn btn-success"><i class="fas fa-plus"></i> Agregar</a>
                </div>
            </div>
        </div>
        <div class="card-body">
            <table class="table table-sm">
                ...
                <tbody>
                    @php
                        $i=0;
                    @endphp
                    @foreach ($destinos_grupo_pregunta->sortBy('pregunta') as $item)
                    @php
                        $i++;
                    @endphp
                    <tr id="lista_{{ $item->id }}">
                        ...
                        <td>
                            <div class="btn btn-group">
                                <a class="btn btn-warning" href="{{ route('admin.destino-grupo.preguntas.edit.path',[$destino_grupo->id,$item->id]) }}"><i class="fas fa-edit"></i></a>
                                <form id="form_borrar_{{ $item->id }}" action="{{ route('admin.destino-grupo.preguntas.destroy.path',[$destino_grupo->id,$item->id]) }}" method="get">
                                    @csrf
