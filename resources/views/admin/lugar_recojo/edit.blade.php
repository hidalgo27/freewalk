...
@endsection
@section('content')
<form action="{{ route('admin.lugar_recojo.update.path',$oLugar_recojo->id) }}" method="post" enctype="multipart/form-data">
    <div class="card">
            ...
            <div class="form-row">
                <div class="form-group col-6">
                    ...
                    <select  class="form-control" id="idioma" name="idioma" onchange="mostrar_destinos($(this).val(),'editar')">
                        ...
                        <option value="es" @if ($oLugar_recojo->idioma=='es') selected @endif>Español</option>
                        <option value="en" @if ($oLugar_recojo->idioma=='en') selected @endif>Ingles</option>
                    </select>
                </div>
                <div class="form-group col-6">
                    ...
                    <select  class="form-control" id="destino" name="destino" >
                        ...
                        @foreach ($oDestinos->sortBy('nombre') as $destino)
                            <option value="{{ $destino->id }}" @if ($oLugar_recojo->destino_id==$destino->id) selected @endif>{{ $destino->nombre }}</option>
                        @endforeach
                    </select>
                </div>
