        ...
        <ul class="navbar-nav p-2">
            @foreach($destino_inicio_idiomas as $destinos_inicio_idiomas)
                @if(strtolower($destinos_inicio_idiomas->idioma) !== strtolower($locale))
                    <li><a class="flag" href="{{ route('lang_path', strtolower($destinos_inicio_idiomas->idioma)) }}"><img src="{{asset('images/'.strtolower($destinos_inicio_idiomas->idioma).'.png')}}" class="mx-2" alt="flag spanish"></a></li>
                @endif
            @endforeach
{{--            @if ($locale == 'en')--}}
